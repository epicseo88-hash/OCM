...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <svg key={i} width="14" height="14" viewBox="0 0 24 24" fill={i <= Math.round(rating) ? '#f59e0b' : '#374151'}>
          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
        </svg>
      ))}
      <span className="ml-1 text-xs font-semibold" style={{ color: '#f59e0b' }}>{rating.toFixed(1)}</span>
    </div>
  )
}
          ...
          >
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ width: 40, height: 40, background: color + '22', color, border: `1.5px solid ${color}44` }}
              >
                {initials}
              </div>
              <div>
            ...
            <p className="text-sm" style={{ color: 'var(--text-muted)', lineHeight: 1.6 }}>{t.comment}</p>
            <p className="text-xs" style={{ color: 'var(--text-muted)', opacity: 0.5 }}>{t.date}</p>
          </div>
        )
      })}
    </div>
